...
import WageGarnishment from "../WageGarnishment";
...
describe.skip("generateFiles", () => {
  it("creates a file for each template on the document", async () => {
    const files = await WageGarnishment.generateFiles(fullData);

    expect(files.length).toEqual(WageGarnishment.templates.length);
  });
});
