...
import FalseCertAbilityToBenefit from "../FalseCertAbilityToBenefit";
...
describe.skip("generateFiles", () => {
  it("creates a file for each template on the document", async () => {
    const files = await FalseCertAbilityToBenefit.generateFiles(fullData);

    expect(files.length).toEqual(FalseCertAbilityToBenefit.templates.length);
  });
});
