...
import UnauthorizedSignForm from "../UnauthorizedSignForm";
...
describe.skip("generateFiles", () => {
  it("creates a file for each template on the document", async () => {
    const files = await UnauthorizedSignForm.generateFiles(fullData);

    expect(files.length).toEqual(UnauthorizedSignForm.templates.length);
  });
});
